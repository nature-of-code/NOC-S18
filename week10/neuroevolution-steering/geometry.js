...
function intersecting(a, b, c, r) {
  ...
  let d = p5.Vector.sub(b, a); //createVector((b.x - a.x) / abdist, (b.y - a.y) / eDistAtoB);
  d.normalize();
  ...
  let e = createVector((t * d.x) + a.x, (t * d.y) + a.y);

  // Calculate the euclidean distance between c & e
  let distCE = p5.Vector.dist(e, c);
...
function is_on(a, b, c) {
  let d1 = p5.Vector.dist(a, c) + p5.Vector.dist(c, b);
  let d2 = p5.Vector.dist(a, b);
  let diff = abs(d1 - d2);
  return diff < 0.01;
}